...
public class FindFontOp extends AbstractGraphicOperator {
    ...
    public void interpret(Context context) {
        ...
        if (oName == null || !(oName.getType() == Type.NAME || oName.getType() == Type.STRING) || oName.isExecutable()) {
            context.pushToOperandStack(oName);
            return;
        }
        PSObject newName = oName;
        if(oName.getType() == Type.NAME){
            String nameFont = ((PSName) oName.getValue()).getStrValue();
            newName = new PSObject(new PSString( nameFont ));
        }
